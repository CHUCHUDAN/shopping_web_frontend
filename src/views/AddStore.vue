<template>
  ...
  <div class="wrapper">
    <div class="form-wrapper">
      ...
      <form @submit.prevent="postStore" :class="storeForm.formClass" novalidate>
        ...
        <div class="form-item">
          ...
          <select class="form-input" v-model="storeForm.category"  name="category" id="category" required>
            ...
            <option class="form-input" :value="item.id" v-for="(item) in storeStore.categories" v-bind:key="item.id">{{ item.name }}
            </option>
          </select>
        ...
        <div>
          <ButtonComponent msg="上架" backgroundColor="background-color:#FFBD9D" type="submit" @click="storeForm.addFormClass()">
          </ButtonComponent>
        </div>
      </form>
    </div>
...
// 取得所有商品分類api
onMounted(async () => {
  await storeStore.getCategories()
})
...
// 上架商品api
const postStore = async (e) => {
  const form = e.target
  // 表單驗證不過
  if (!form.checkValidity()) return e.preventDefault()
  const postStoreResult = await storeStore.postStore()
  if (postStoreResult) return
